                        ...
                        @if ($project->reports->count() > 0)
                            @if (auth()->user()->role->id == \App\Models\Role::IS_DEPUTY_PROJECT_MANAGER && $project->status == 'pending_1')
                                <form action="{{ route('project.approve_project_procurator') }}" method="post">
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="text-lg text-white rounded shadow px-4 py-2 tracking-tight bg-green-500">
                                        موافقة
                                    </button>
                                </form>
                                <form action="{{ route('project.decline_project_procurator') }}" method="post">
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="text-lg text-white rounded shadow px-4 py-2 tracking-tight bg-red-500">
                                        رفض
                                    </button>
                                </form>
