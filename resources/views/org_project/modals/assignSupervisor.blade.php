    ...
    <div class="bg-white shadow-xl rounded p-2">
        ...
        <div class="py-4 px-8 ">
            ...
            <div class="flex flex-col gap-4 mt-5 max-h-[500px] overflow-y-auto">
                <form method="POST" action="{{ route('assign_supervisor') }}">
                    @csrf
                    ...
                    <select name="supervisor_id" id="supervisor_id" class="border-2 border-gray-400 w-full">
                        @foreach (\App\Models\User::where('role_id', \App\Models\Role::IS_SUPERVISOR)->get() as $item)
                            <option value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                    <button type="submit"
                        class="bg-[#673B8C] text-white px-4 py-2 rounded-xl text-lg w-full mt-3 shadow-lg">{{ __('تعيين مشرف') }}</button>
                </form>
            </div>
        </div>
    </div>
