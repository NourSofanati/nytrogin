            ...
            <div class="flex flex-col gap-4 mt-5 max-h-[500px] overflow-y-auto">
                <form method="POST" action="{{ route('assign_dep') }}">
                    ...
                    <select name="dpm_id" id="dpm_id" class="border-2 border-gray-400 w-full">
                        @foreach (\App\Models\User::where('role_id', \App\Models\Role::IS_PROJECT_MANAGER)->get() as $item)
                            <option value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                    <button type="submit"
                        class="bg-[#673B8C] text-white px-4 py-2 rounded-xl text-lg w-full mt-3 shadow-lg">{{ __('تعين مدير المشروع') }}</button>
                </form>
            </div>
        </div>
    </div>
